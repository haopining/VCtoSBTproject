...
import { createAnonDecrypter, createAnonEncrypter, createJWE, decryptJWE } from 'did-jwt';
...
export class KeyManager {
    ...
    async keyManagerGetKeyManagementSystems() {
    ...
    async keyManagerEncryptJWE({ kid, to, data }) {
        ...
        const result = await createJWE(dataBytes, [encrypter]);
        return JSON.stringify(result);
    }
    /** {@inheritDoc @veramo/core-types#IKeyManager.keyManagerDecryptJWE} */
    async keyManagerDecryptJWE({ kid, data }) {
    ...
    async keyManagerSignJWT({ kid, data }) {
    ...
    async keyManagerSign(args) {
        ...
        const keyInfo = await this.store.getKey({ kid: keyRef });
        let dataBytes;
        if (typeof data === 'string') {
    ...
    async keyManagerSignEthTX({ kid, transaction }) {
    ...
    async keyManagerSharedSecret(args) {
    ...
    createX25519ECDH(secretKeyRef) {
        return async (theirPublicKey) => {
            ...
            const publicKey = { type: 'X25519', publicKeyHex: hexlify(theirPublicKey).substring(2) };
